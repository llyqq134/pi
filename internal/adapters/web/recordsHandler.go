...
func (h *recordsHandler) AddRecordHandler(c *gin.Context) {
	...
	if err := c.BindJSON(&record); err != nil {
		log.Printf("error while binding record playload: %v\n", err)
	...
	newRecord, err := h.recordsService.CreateRecord(record.EquipmentId, workerID, workerName, departmentID, departmentName, record.ExpectedReturnDate, record.Status)
	if err != nil {
...
func generateRecordsPDF(records []entities.Records, startDate, endDate time.Time) *gofpdf.Fpdf {
	...
	pdf.Cell(0, 10, "Отчет по выдаче оборудования")
	pdf.Ln(8)

	pdf.SetFont("Arial", "", 12)
	pdf.CellFormat(0, 6, fmt.Sprintf("Период: %s - %s",
		startDate.Format("02.01.2006"),
		endDate.Format("02.01.2006")),
		"", 1, "L", false, 0, "")
	pdf.Ln(5)

	pdf.CellFormat(0, 6, fmt.Sprintf("Всего записей: %d", len(records)),
		"", 1, "L", false, 0, "")
	pdf.Ln(5)

	pdf.SetFont("Arial", "B", 9)

	widths := []float64{10, 28, 35, 35, 28, 28, 28, 28}

	headers := []string{"№", "Оборудование", "Работник", "Отдел", "Дата выдачи", "Дата возврата", "Ожидаемая дата", "Статус"}
	...
	for i, record := range records {
		...
		pdf.CellFormat(widths[1], 7, record.EquipmentId, "1", 0, "L", false, 0, "")
		...
		pdf.CellFormat(widths[7], 7, formatStatus(record.Status), "1", 0, "C", false, 0, "")
		pdf.Ln(7)
	}

	pdf.SetY(-15)
	pdf.SetFont("Arial", "I", 8)
	pdf.CellFormat(0, 10, fmt.Sprintf("Сформировано: %s", time.Now().Format("02.01.2006 15:04")),
		"T", 0, "R", false, 0, "")
