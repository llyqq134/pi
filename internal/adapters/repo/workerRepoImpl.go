...
func (r *WorkerRepoImpl) Create (ctx context.Context, worker *entities.Worker) error {
	...
	if err := r.Client.QueryRow(ctx, query, 
		&worker.Name, &worker.JobTitle, &worker.Department, &worker.HashPass, &worker.AcessLevel).Scan(&worker.UUID);
		err != nil {
...
func (r * WorkerRepoImpl) GetAll(ctx context.Context) ([]entities.Worker, error) {
	...
	for rows.Next() {
		var worker entities.Worker
		if err := rows.Scan(&worker.UUID, &worker.Name, &worker.JobTitle, &worker.Department, &worker.AcessLevel);
			err != nil {
